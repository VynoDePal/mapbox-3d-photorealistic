...
import { initLightPresetBar, getStoredPreset } from './light-preset.js';
import { reverse, GeocodingError } from './geocoding.js';
...
function showFatalError(title, ...lines) {
  ...
  inner.innerHTML = `<h1>${title}</h1>` +
    lines.map((l) => l.startsWith('VITE_') ? `<code>${l}</code>` : `<p>${l}</p>`).join('');
  ...
  map.on('error', (e) => {
    if (e.error?.status === 401) {
      showFatalError(
        ...
        'Vérifie que VITE_MAPBOX_ACCESS_TOKEN dans .env contient un token public valide (préfixe pk.).'
      );
    }
  });
...
function initClickReverseGeocode(map) {
  let abortCtrl;
    ...
    try {
      ...
      popup.setHTML(
        `<div><strong>${escapeHtml(name)}</strong></div>` +
          (fullAddress ? `<div style="color:var(--text-dim);margin-top:4px;font-size:12px">${escapeHtml(fullAddress)}</div>` : '')
      );
    } catch (err) {
