...
| BUG-009 (v2) | 🔵 Cosmétique | Preset URL invalide | ✅ DONE | _pending_ | `parseHashWithErrors` expose désormais `invalidPresetValue` (la chaîne invalide brute). main.ts boot affiche toast 6s **immédiat** (pas de setTimeout 500ms) avec message nominatif `presetInvalidNamed(invalid, fallback)`. `history.replaceState` nettoie le hash en remplaçant le segment preset par le défaut, évite que les `moveend` re-poussent l'invalide. |
...
| BUG-011 (v2) | 🔵 Cosmétique | Story ne masque pas Itin/Favoris | ✅ DONE | _pending_ | `initStory` accepte un 3e arg `SidePanels { favorites?, directions? }`. À l'`activate()`, snapshot des `isOpen()` + close des panneaux ouverts. À la `deactivate()`, restore via `open()` / `showPanel()`. `initFavoritesPanel` retourne désormais un `FavoritesController { isOpen, open, close }`. `DirectionsController` a `isPanelOpen / hidePanel / showPanel` (cf. BUG-003 v2). Wire dans main.ts. |
| BUG-012 (v2) | 🔵 Cosmétique | Auto preset peu visible | ✅ DONE | _pending_ | (1) CSS spécifique `[data-preset="auto"].active` (override le selector data-attr `::before`). (2) Classe `.preset-auto-driven` (outline dashed accent) ajoutée au bouton du preset que Auto pilote — visuel clair de ce qui est piloté. (3) `aria-label` dynamique : `Mode Auto activé — preset actuel : Jour` quand on, `Mode Auto` quand off. Re-render au langChange. |
